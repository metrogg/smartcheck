...
| 4 | `/api/employees/import` | POST | 批量导入员工（含照片） |
| 5 | `/api/employees/{employeeId}` | DELETE | 删除员工 |
| 6 | `/api/employees/clear-all` | DELETE | 清空所有员工 |
| 7 | `/api/records` | GET | 批量查询晨检记录 |
| 8 | `/api/records/{id}` | GET | 查询单条记录详情 |
| 9 | `/api/records/sync` | GET | 增量同步 |
| 10 | `/api/records/statistics` | GET | 统计汇总 |
| 11 | `/api/records/export` | POST | 导出 CSV 文件 |
| 12 | `/api/images/{filename}` | GET | 下载晨检图片 |
| 13 | `/api/employee-images/{filename}` | GET | 下载员工照片 |
| 14 | `/api/users/sync` | POST | 账号信息同步 |
